...
vector<GLfloat> Mesh::transform() {
	...
	for (size_t i = 0; i < vFace.size(); i++) {
		for (size_t j = 0; j < 3; j++) {
			...
			final.push_back(vNorm[t * 3]);
			final.push_back(vNorm[t * 3 + 1]);
			final.push_back(vNorm[t * 3 + 2]);
		}
...
vector<GLfloat> Mesh::transformIndecise() {  //vertex without color, in ordering
	...
	for (size_t i = 0; i < vVert.size(); i++) {
		...
		final.push_back(t);
		if (i % 6 == 5) {
			final.push_back(vNorm[i / 6 * 3]);
			final.push_back(vNorm[i / 6 * 3 + 1]);
			final.push_back(vNorm[i / 6 * 3 + 2]);
		}
